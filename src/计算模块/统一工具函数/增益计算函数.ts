...
export const 获取装备增益 = (
  ...
  if (!快照计算?.includes('大附魔_伤腰')) {
    if (装备信息?.装备增益?.大附魔_伤腰) {
      ...
      const 装备特效增益效果 = 获取装备特效增益数据('大附魔_伤腰', 1, 装备增益数据, 腰带覆盖率)
      if (装备特效增益效果) {
        总增益集合 = 总增益集合.concat(装备特效增益效果)
      }
    }
...
export const 根据增益修改最终循环 = (
  ...
  if (装备信息?.装备增益?.特效_42767) {
    if (!最终循环?.some((item) => item.技能名称?.includes('速·震'))) {
      ...
      const 实际伤害名称 = 装备信息?.装备增益?.特效_42767 > 1 ? `速·震·2` : `速·震`
      最终循环.push({
        技能名称: 实际伤害名称,
  ...
  if (装备信息?.装备增益?.特效_40788) {
    if (!最终循环?.some((item) => item.技能名称?.includes('巽') || item.技能名称 === '特效·腕')) {
      const 实际伤害名称 = 最终循环?.some((item) => item.技能名称 === '特效·腕')
        ? '特效·腕'
        : `巽·${装备信息?.装备增益?.特效_40788 || 1}`
      最终循环.push({
        技能名称: 实际伤害名称,
        技能数量:
          最终循环?.find((item) => item.技能名称 === '特效·腕')?.技能数量 ||
          Math.floor(战斗时间 / 10),
      })
    }
  } else {
