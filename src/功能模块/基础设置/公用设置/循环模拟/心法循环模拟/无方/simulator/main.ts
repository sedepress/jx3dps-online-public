...
// // import 磁雷外泄 from './技能类/磁雷外泄'
import 万灵丹 from './技能类/万灵丹'
...
class 循环主类 {
  ...
  消耗内力(数值) {
    ...
    if (this.技能类实例集合.千枝绽蕊?.千枝绽蕊状态校验() && this.角色状态信息.内力 < 6) {
  ...
  千枝结算() {
    const 千枝绽蕊存在 = !!this.当前自身buff列表?.['千枝绽蕊']?.当前层数
    const 千枝伏藏存在 = !!this.当前自身buff列表?.['千枝伏藏']?.当前层数
    ...
    if (!this.下个千枝结算帧) {
      this.下个千枝结算帧 = this.当前时间 + 4
    }

    while (this.当前时间 >= this.下个千枝结算帧) {
      if (this.当前自身buff列表?.['千枝绽蕊']?.当前层数) {
        this.消耗内力(1.25)
      } else if (this.当前自身buff列表?.['千枝伏藏']?.当前层数) {
        this.回复内力(0.25)
      }
      this.下个千枝结算帧 += 4
    }
    ...
    if (this.待生效事件队列.length) {
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          } else if (当前事件.事件名称 === '万灵丹_秒回') {
            this.技能类实例集合?.['万灵丹']?.万灵丹每秒回复()
          }
          this.待生效事件队列?.shift()
          if (this.待生效事件队列.length) {
            轮训执行事件()
