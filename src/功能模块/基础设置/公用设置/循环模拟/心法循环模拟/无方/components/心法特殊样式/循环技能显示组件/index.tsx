...
import BuffItem from '../../../../通用/通用组件/循环技能显示组件/BuffItem'
...
function CycleSkillItem(props: CycleSkillItemProps) {
  ...
  const 判断千枝绽蕊 = () => {
    ...
    if (buffList.includes('千枝绽蕊')) {
      return ' skill-background-qianzhi'
    }
    return ''
  }
  ...
  const 底部标识展示 = useMemo(() => {
    if (底部标识 && 底部标识?.(技能, 技能索引) !== undefined) {
      return 底部标识?.(技能, 技能索引)
    } else {
      return null
    }
  ...
  const 背景容器展示 = useMemo(() => {
    if (背景容器 && 背景容器?.(技能, 技能索引) !== undefined) {
      return 背景容器?.(技能, 技能索引)
    } else {
      return null
    }
  ...
  return 是否隐藏 ? (
    ...
    <Badge
      count={剩余秒}
      offset={[-52, 8]}
      className={'cycle-simulator-setting-skill-drag' + `${判断千枝绽蕊()}`}
    >
