...
function CycleSimulator() {
  ...
  const simulator = (props?) => {
    ...
    if (更新展示) {
      ...
      更新模拟信息({
        ...
        当前目标buff列表: 处理后目标buff,
        ...rest,
        角色状态信息: (rest as any)?.角色状态信息 || 模拟信息?.角色状态信息,
      })
    }
  ...
  const 获取战斗时间 = (data: 循环日志数据类型[], 模拟结束时间: number, 战斗开始时间?: number) => {
    if (战斗开始时间 === undefined) {
      return 0
    }
    const 伤害日志 = (data || []).filter((item) => item?.日志类型 === '造成伤害')
    const 最后伤害时间 = 伤害日志?.length ? 伤害日志?.[伤害日志.length - 1]?.日志时间 || 0 : 0
    // DPS计算只关心“从战斗开始到最后一次造成伤害”为止。
    // 准备阶段（战斗开始前）以及循环末尾可能存在的无伤害等待，不应计入战斗用时。
    return Math.max(最后伤害时间 - 战斗开始时间, 0)
  }
