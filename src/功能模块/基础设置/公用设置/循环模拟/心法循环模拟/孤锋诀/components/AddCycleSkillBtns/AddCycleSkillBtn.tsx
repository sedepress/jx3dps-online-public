...
const AddCycleSkillBtn: React.FC<AddCycleSkillBtnProps> = (props) => {
  ...
  const 断云石消耗锐意 =
    模拟信息?.技能基础数据?.find((item) => item.技能名称 === '断')?.消耗锐意 || 0
  ...
  const 异常信息: 异常信息数据 = useMemo(() => {
    ...
    if (!插入技能) {
      ...
      } else if (技能?.技能名称 === '断' && (模拟信息?.角色状态信息?.锐意 || 0) < 断云石消耗锐意) {
        禁用信息 = {
          是否禁用: true,
      ...
      // else if (
      //   技能?.技能名称 === '弱点击破' &&
      //   ((!模拟信息?.当前目标buff列表?.['弱点_左']?.当前层数 &&
      //     !模拟信息?.当前目标buff列表?.['弱点_右']?.当前层数) ||
      ...
      } else if (技能?.技能名称 === '沧逸' && !模拟信息?.当前自身buff列表?.['倾怒']?.当前层数) {
        禁用信息 = {
          是否禁用: true,
          异常描述: ERROR_ACTION?.BUFF错误?.信息,
        }
      } else if (技能?.技能名称 === '倾怒' && !模拟信息?.当前自身buff列表?.['沧逸']?.当前层数) {
        禁用信息 = {
          是否禁用: true,
          异常描述: ERROR_ACTION?.BUFF错误?.信息,
        }
