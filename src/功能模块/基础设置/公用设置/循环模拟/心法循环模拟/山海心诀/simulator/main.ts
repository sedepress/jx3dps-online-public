...
class 循环主类 {
  ...
  技能造成伤害(
    ...
    // 当前技能数据?.宠物伤害
    if (this.校验奇穴是否存在('卢令') && 总增益列表?.some((item) => item?.includes('承契'))) {
      总增益列表.push('诸怀')
    }
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          if (当前事件.事件名称?.includes('没石饮羽')) {
            ...
            this?.技能类实例集合?.下马?.命中()
          } else if (当前事件.事件名称?.includes('领胡攻击')) {
            this?.技能类实例集合?.汇灵合契?.领胡攻击()
          }
          this.待生效事件队列?.shift()
          if (this.待生效事件队列.length) {
            轮训执行事件()
  ...
  标鹄判定() {
    ...
    const 当前连珠层数 = this.当前自身buff列表?.['连珠']?.当前层数 || 0
    // 引爆标鹄
    if (当前标鹄层数 === 4) {
      // 引爆上一层贯穿
    ...
    if (this.校验奇穴是否存在('襄尺')) {
      ...
      if (buff承契数据) {
        ...
        if (buff承契数据?.当前层数 === 5 && 剩余帧 >= 每秒郭氏帧 * 8) {
          this.技能类实例集合.金乌见坠.减少调息时间(每秒郭氏帧 * 3, '触发襄尺')
        }
      }
    }
