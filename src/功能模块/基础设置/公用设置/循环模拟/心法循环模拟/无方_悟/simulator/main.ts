...
import 钩吻断肠普通 from './技能类/钩吻断肠普通'
import 钩吻断肠追加 from './技能类/钩吻断肠追加'
import 川乌射罔悟 from './技能类/川乌射罔悟'
import 青圃着尘悟 from './技能类/青圃着尘悟'
import 沾衣未妨悟 from './技能类/沾衣未妨悟'
import 并蒂夺株悟 from './技能类/并蒂夺株悟'
import 苍棘缚地悟 from './技能类/苍棘缚地悟'
import 凌然天风悟 from './技能类/凌然天风悟'
import 唤醒 from './技能类/唤醒'
import 易命 from './技能类/易命'
import 损毁 from './技能类/损毁'
import 特效腰坠 from '../../通用/通用技能/特效腰坠'

import 中毒 from './DOT类/中毒'
...
class 循环主类 {
  ...
  初始化技能实例类() {
    this.技能类实例集合 = {
      '钩吻断肠·悟·普通': new 钩吻断肠普通(this),
      '钩吻断肠·悟·追加': new 钩吻断肠追加(this),
      '川乌射罔·悟': new 川乌射罔悟(this),
      '青圃着尘·悟': new 青圃着尘悟(this),
      '沾衣未妨·悟': new 沾衣未妨悟(this),
      '凌然天风·悟': new 凌然天风悟(this),
      '并蒂夺株·悟': new 并蒂夺株悟(this),
      '苍棘缚地·悟': new 苍棘缚地悟(this),
      唤醒: new 唤醒(this),
      易命: new 易命(this),
      损毁: new 损毁(this),
      中毒: new 中毒(this),
      ...
      const 轮训执行事件 = () => {
        if (this.待生效事件队列[0]?.事件时间 <= 结算判断时间) {
          ...
          if (当前事件.事件名称?.includes('技能读条')) {
            ...
            const 技能名称 = 当前事件?.事件备注?.技能名称 || '沾衣未妨·悟'
            this.技能类实例集合?.[技能名称]?.沾衣未妨结算(当前事件?.事件备注?.香繁饮露触发)
          } else if (当前事件.事件名称 === '并蒂夺株') {
            const 技能名称 = 当前事件?.事件备注?.技能名称 || '并蒂夺株·悟'
            this.技能类实例集合?.[技能名称]?.并蒂夺株结算()
          } else if (当前事件.事件名称 === '苍棘缚地攻击') {
            const 技能名称 = 当前事件?.事件备注?.技能名称 || '苍棘缚地·悟'
            this.技能类实例集合?.[技能名称]?.苍棘缚地攻击()
          }
          this.待生效事件队列?.shift()
          if (this.待生效事件队列.length) {
            轮训执行事件()
