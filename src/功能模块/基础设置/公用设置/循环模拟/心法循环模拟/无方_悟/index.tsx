...
function CycleSimulator() {
  ...
  const [起手温寒, 更新起手温寒] = useState<number>(-3) // 3寒
  const [启用斩杀, 更新启用斩杀] = useState<boolean>(false)
  const [斩杀起始秒数, 更新斩杀起始秒数] = useState<number>(0)
    ...
    const newCycle: 循环基础技能数据类型[] = cycle?.map((item, index) => {
      ...
      if (index === 索引) {
        if (延迟) {
          ...
          if (newItem?.额外信息) {
            delete newItem.额外信息.延迟
            if (Object.keys(newItem.额外信息).length === 0) {
              delete newItem.额外信息
            }
          }
        }
