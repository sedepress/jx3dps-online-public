import { 获取实际技能数据 } from '../../../通用/通用函数'
import 循环主类类型 from '../main'
import { 循环基础技能数据类型, 技能释放记录结果 } from '../type'

class 技能统一类 {
  模拟循环: Partial<循环主类类型> = {}
  本次释放记录: 技能释放记录结果 = {}
  本次释放延迟: number | 'GCD' = 0
  技能释放时怒气: number = 0

  static 绝刀怒气判定 = {
    绝返等级: 0,
    绝破等级: 0,
    消耗怒气: 0,
  }

  static 释放时援戈层数 = 0
  static 释放前怒气 = 0
  constructor(模拟循环) {
    this.模拟循环 = 模拟循环
    return
  }

  获取实际GCD(当前技能?: 循环基础技能数据类型) {
    if (!当前技能) {
      return 0
    }
    const 应校验GCD组: string[] = 当前技能?.检测GCD组 || []
    let 最大GCD = 0
    应校验GCD组.forEach((GCD组) => {
      let 实际GCD = 0
      if (GCD组 === '自身') {
        实际GCD = this.模拟循环.GCD组?.[当前技能?.技能名称]
      } else {
        实际GCD = this.模拟循环.GCD组?.[GCD组]
      }
      if (实际GCD > 最大GCD) {
        最大GCD = 实际GCD
      }
    })
    return 最大GCD
  }

  检查GCD(索引, 当前技能?: 循环基础技能数据类型) {
    const 后一个技能 = this.模拟循环?.测试循环?.[索引 + 1]
    let GCD = this.获取实际GCD(当前技能)
    let 等待GCD = 0
    if (this.本次释放延迟) {
      if (this.本次释放延迟 === 'GCD' && 后一个技能) {
        const 下一个技能实际数据: any = 获取实际技能数据(后一个技能)
        const 下一个技能 = this?.模拟循环?.技能基础数据?.find(
          (item) => item?.技能名称 === 下一个技能实际数据?.实际技能名称,
        )
        if (下一个技能 && (下一个技能?.检测GCD组 || 下一个技能?.技能GCD组)) {
          if (
            (下一个技能?.检测GCD组?.length || 下一个技能?.技能GCD组) &&
            下一个技能实际数据?.额外信息?.延迟 !== 'GCD'
          ) {
            const 技能实例 = this?.模拟循环?.技能类实例集合?.[下一个技能?.技能名称]
            const 后一个技能GCD = this.模拟循环?.检查GCD?.(下一个技能, 技能实例, 索引 + 1) || 0
            等待GCD = 后一个技能GCD
          } else {
            const 再下一个技能实际名称 = 获取实际技能数据(
              this.模拟循环?.测试循环?.[索引 + 2],
            )?.实际技能名称
            const 再下一个技能 = this?.模拟循环?.技能基础数据?.find(
              (item) => item?.技能名称 === 再下一个技能实际名称,
            )
            if (再下一个技能 && (再下一个技能?.检测GCD组?.length || 再下一个技能?.技能GCD组)) {
              const 技能实例 = this?.模拟循环?.技能类实例集合?.[再下一个技能?.技能名称]
              const 后一个技能GCD = this.模拟循环?.检查GCD?.(再下一个技能, 技能实例, 索引 + 2) || 0
              等待GCD = 后一个技能GCD
            }
          }
        }
      } else {
        等待GCD = Number(this.本次释放延迟)
      }
      if (等待GCD) {
        GCD = Math.max(等待GCD, GCD)
      }
    }

    this.本次释放延迟 = 0
    return GCD
  }

  统一初始化(额外信息) {
    if (额外信息?.延迟) {
      this.本次释放延迟 = 额外信息?.延迟
    }
    // 重置释放记录
    this.本次释放记录 = {}
    this.技能释放时怒气 = this.模拟循环?.角色状态信息?.怒气 || 0
  }
  触发回复怒气(怒气值, 来源) {
    this.模拟循环.回复怒气?.(怒气值, 来源)
  }
  触发消耗怒气(怒气值, 来源) {
    this.模拟循环.消耗怒气?.(怒气值, 来源)
  }
  // 触发回复格挡(格挡值, 来源) {
  //   this.模拟循环.回复格挡?.(格挡值, 来源)
  // }
  // 触发消耗格挡(格挡值, 来源) {
  //   this.模拟循环.消耗格挡?.(格挡值, 来源)
  // }

  触发伤害行为(
    伤害名称,
    伤害次数 = 1,
    额外增益列表: string[] = [],
    触发伤害时间: number | undefined = undefined,
    DOT伤害 = false,
    技能等级 = 1,
  ) {
    this.模拟循环.技能造成伤害?.(伤害名称, 伤害次数, 额外增益列表, 触发伤害时间, DOT伤害, 技能等级)

    this.模拟循环.卸除buff?.({ 名称: '弱点内置CD', 对象: '目标' })
  }

  重置苍雪刀调息时间() {
    // 重置苍雪刀调息时间
    const 苍雪刀技能列表 = ['斩刀', '绝刀', '劫刀', '闪刀']

    苍雪刀技能列表.forEach((技能名称) => {
      const 技能实例 = this.模拟循环?.技能类实例集合?.[技能名称]
      技能实例?.重置调息时间?.()
    })
  }
  重置斩刀调息时间() {
    // 重置斩刀调息时间
    const 斩刀技能名称 = '斩刀'
    const 技能实例 = this.模拟循环?.技能类实例集合?.[斩刀技能名称]
    技能实例?.重置调息时间?.()
  }

  重置盾压调息时间() {
    // 重置盾压调息时间
    const 盾压技能名称 = '盾压'
    const 技能实例 = this.模拟循环?.技能类实例集合?.[盾压技能名称]
    技能实例?.重置调息时间?.()
  }

  记录绝刀怒气判定() {
    const { 绝返等级, 绝破等级, 消耗怒气 } =
      this.模拟循环.技能类实例集合?.['绝刀']?.判定绝刀怒气消耗?.() || {}
    技能统一类.绝刀怒气判定 = {
      绝返等级,
      绝破等级,
      消耗怒气,
    }
    return 技能统一类.绝刀怒气判定
  }

  // 用来在模拟器内显示援戈层数标记。
  记录释放时援戈层数() {
    技能统一类.释放时援戈层数 = this.模拟循环.当前自身buff列表?.['援戈']?.当前层数 ?? 0
    return 技能统一类.释放时援戈层数
  }
  获取释放时援戈层数() {
    return 技能统一类.释放时援戈层数 || undefined
  }
  // 特殊标记在大于10之后会显示为灰色，并且最多只显示两位数，把1-10层的援戈都显示为10倍并且在1-9前面加一个0，从而让援戈层数标记变成01-12，且统一为灰色
  格式化援戈标记(层数: number): string {
    const 数值 = 层数 * 10
    if (数值 < 100) {
      // return '\u00A0' + 数值.toString().padStart(2, '0') // 这样会显示一个空格，虽然没有多余的0了，但是空出来不太好看
      return '0' + 数值.toString().padStart(2, '0')
    }
    return 数值.toString()
  }

  触发血誓效果(绝返等级 = 0) {
    // 检查是否拥有血怒或血怒·惊涌buff
    const 有血誓奇穴 = this.模拟循环.校验奇穴是否存在?.('血誓') || 0
    if (!有血誓奇穴) {
      return // 如果没有点出血誓奇穴，则不触发血誓效果
    }
    const 有血怒 = (this.模拟循环.当前自身buff列表?.['血怒']?.当前层数 ?? 0) > 0
    const 有血怒惊涌 = (this.模拟循环.当前自身buff列表?.['血怒_惊涌']?.当前层数 ?? 0) > 0
    if (有血怒 || 有血怒惊涌) {
      const 血誓层数 = this.模拟循环.当前目标buff列表?.['以血盟誓']?.当前层数 || 0
      if (!血誓层数 || 血誓层数 === 1) {
        this.模拟循环.添加buff?.({ 名称: '以血盟誓', 对象: '目标', 新增层数: 1 })
      } else if (血誓层数 === 2) {
        this.模拟循环.卸除buff?.({ 名称: '以血盟誓', 对象: '目标', 卸除层数: 2 })
        this.触发伤害行为('血誓', 1, 绝返等级 ? [`绝返·${绝返等级}`] : [], undefined, false, 1)
      }
    }
  }
  获取技能释放记录结果() {
    return {
      ...this.本次释放记录,
    }
  }

  获取当前重要buff列表(技能依赖自身buff列表: string[] = [], 技能依赖目标buff列表: string[] = []) {
    const 重要buff列表: string[] = []
    技能依赖自身buff列表.forEach((buff) => {
      if (this.模拟循环.当前自身buff列表?.[buff]?.当前层数) {
        重要buff列表.push(buff)
      }
    })
    技能依赖目标buff列表.forEach((buff) => {
      if (this.模拟循环.当前目标buff列表?.[buff]?.当前层数) {
        重要buff列表.push(buff)
      }
    })
    return 重要buff列表 || []
  }

  获取施加重要buff信息(buff名称) {
    const 当前时间 = this.模拟循环.当前时间 || 0
    const buff对象 = this.模拟循环.Buff和Dot数据?.[buff名称]

    return buff对象
      ? {
          buff名称: buff名称,
          buff开始时间: 当前时间,
          buff结束时间: 当前时间 + (buff对象?.最大持续时间 || 0),
        }
      : null
  }

  // 弱点击破标记() {
  //   // 标记该技能可以弱点击破
  //   if (this.模拟循环.当前目标buff列表?.['弱点']?.当前层数) {
  //     this.模拟循环.添加buff?.({ 名称: '可击破', 对象: '目标' })
  //   }
  // }
}

export default 技能统一类
