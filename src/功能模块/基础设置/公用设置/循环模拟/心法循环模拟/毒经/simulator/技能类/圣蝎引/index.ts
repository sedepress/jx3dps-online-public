...
class 圣蝎引 extends 有CD技能通用类 {
  static 技能数据 = 循环模拟技能基础数据?.find((item) => item.技能名称 === '圣蝎引')

  constructor(模拟循环) {
    super(模拟循环)
    圣蝎引.技能数据 = 模拟循环?.技能基础数据?.find((item) => item.技能名称 === '圣蝎引')
    this.初始化技能运行数据()
  }
  ...
  命中() {
    ...
    if (this.模拟循环.当前自身buff列表?.['灵蛇存在']?.当前层数) {
      this.模拟循环.卸除buff({ 名称: '灵蛇存在', 对象: '自身', 事件时间: this.模拟循环.当前时间 })
    }
