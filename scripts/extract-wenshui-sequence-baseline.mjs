...
const 读取基线 = (工作表, 配置) => {
  ...
  for (let 来源行 = 3; 来源行 <= 最大行; 来源行 += 1) {
    ...
    if (!原始技能) continue
    const 权重 = Number(工作表[`${配置.权重列}${来源行}`]?.v ?? 0)
    if (!Number.isFinite(权重)) {
      throw new Error(`无效权重: ${配置.权重列}${来源行}`)
    }
    const 技能 = String(原始技能)
    if (!已知技能Token.has(技能)) {
      throw new Error(`未知技能 token: ${原始技能} (${配置.技能列}${来源行})`)
    }
    结果.push([技能, 权重, 来源行])
  }
